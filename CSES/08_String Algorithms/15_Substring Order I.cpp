...
/*
You are given a string of length n. If all of its distinct substrings are ordered lexicographically,
...
int32_t main()
{
    ...
    {   
        ...
        for (int i=0; i<n; i++){
            ...
            if (curr + (n-suffix[i]) - prev >= k) {
                int j = prev;
                string ans = s.substr(suffix[i], j);
