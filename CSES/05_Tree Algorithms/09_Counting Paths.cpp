...
Finally, there are m lines describing the paths. Each line contains two integers a and b: there
is a path between nodes a and b.
*/

/*
Prefix Sum on Tree :-
prefix Sum of leaf node is as it is.
Start from leaf  & prefixSum[node] = prefixSum of all its child.
 
Counting Paths. 
We have to find, for each node -> the no. of path it is been part of after the drawing each path from queries.
 
Brute Force is O(q*n) : Go from a->b and add +1  to the path.
PrefixSum approach    : Add +1 to node[a] & node[b] and add -1 to lca(a,b) and -1 to par[lca(a,b)].  
                      : Run PrefixSum-dfs after all queries.
*/
...
void prefixDfs(int node, vi &vis){
    vis[node] = 1;
    for(int child : adj[node]){
        if(!vis[child]){
            prefixDfs(child, vis);
