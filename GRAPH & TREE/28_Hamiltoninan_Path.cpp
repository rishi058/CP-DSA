...
Given is a Directed Graph.

We need to find a Hamiltonian Path which starts @ 1 and ends @ n.

IDEA :-
Path in which each vertex iss visited exactly once is known as Hamiltonian Path
And if we need the start and the end to be same it is called Hamiltonian Cycle.

Topological sorting can only find a Hamiltonian path in a directed acyclic graph. 
For directed graphs in general, determining whether or not a Hamiltonian path exists is NP-complete
