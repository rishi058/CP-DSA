...
/*
With only Binary Search we can't solve it, 
we have to iterate through the range[i,j] to find min_idx

Therefore we will use min_segment_tree to find min of range in Log(n) time.
*/
    ...
    while(T--)
    {
        ...
        while(q--){
            ...
            if(ind1==-1){cout<<"-1\n";}
                ...
                int ans = tree.query(ind1+1, ind2)+1;
                cout<<ans<<"\n";
            }            
        }
